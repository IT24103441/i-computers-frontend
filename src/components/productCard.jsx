...
export default function ProductCard({ product }) {
    ...
    const rawLabelledPrice = Number(product.labelledPrice ?? product.labeledPrice ?? 0);
    const rawPrice = Number(product.price ?? 0);
    const hasDiscount = rawLabelledPrice > rawPrice;
    const discountPercent = hasDiscount ? Math.round(((rawLabelledPrice - rawPrice) / rawLabelledPrice) * 100) : 0;
    ...
    return (
        <div className={`border w-64 rounded-xl shadow-md p-4 bg-white flex flex-col justify-between transition-all hover:shadow-lg ${isOutOfStock ? "opacity-80" : ""}`}>
                ...
                {product.images?.[0] ? (
                    <img
                        ...
                        onError={(e) => {
                            ...
                            e.target.src = "https://placehold.co/400x300/1e293b/amber?text=Tech+Product";
                        }}
                    />
                ) : (
            ...
            <div className="mb-3 flex items-baseline gap-2">
                <span className="text-amber-600 font-bold text-lg">{getFormattedPrice(rawPrice)}</span>
                {hasDiscount && (
                    <span className="text-xs text-gray-400 line-through">
                        {getFormattedPrice(rawLabelledPrice)}
                    </span>
                )}
            </div>
